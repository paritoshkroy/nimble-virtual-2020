...
locs <-  as.matrix(respiratorydata_sp@data[ , c('easting', 'northing')])
...
conf$addSampler('beta_x[1:39]', 'RW_block', control = list(adaptFactorExponent = 0.25))
...
par(mfrow = c(2,3))
ts.plot(samples[ , 'lambda[1]'], main = 'lambda[1]')
ts.plot(samples[ , 'lambda[2]'], main = 'lambda[2]')
ts.plot(samples[ , 'beta_z'], main = 'beta_z')
ts.plot(samples[ , 'x[5]'], main = 'x[5]')
ts.plot(samples[ , 'x[50]'], main = 'x[50]')
ts.plot(samples[ , 'x[100]'], main = 'x[100]')
```
...
```{r, fig.cap='', fig.width=12, fig.height=7}
xCols <- grep('^x\\[', colnames(samples))
xEstSpl <- colMeans(samples[ , xCols])
...
Let's pull together setting up post-hoc posterior sampling of the latent process at unobserved locations, $x^*$. We'll continue to use the Scotland data, but it's artificial in that with areal data settings, one generally doesn't have new locations. But in more standard GP settings, this is a standard problem.
