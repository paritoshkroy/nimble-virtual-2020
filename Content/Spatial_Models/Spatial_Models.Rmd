...
Notes:
  ...
  - We could observe $y_i$ at point locations (geostatistics) or in areal regions (e.g., zip codes, provinces, etc.).
...
This induces a covariance for the data that combines the measurement error, $\sigma^2$ and the spatial structure, $C(\theta)$.
...
Marginalization:
  ...
  - Sampling non-marginalized models is slow because of:
      - slow computation of spatial process prior (unless can use sparsity)
      - high-dimensional parameter space to explore (regardless of sparsity)

One-at-a-time vs. block sampling: sample $x$ as a vector of $x_i|x_{-i}$?
  - Joint sampling requires ways to make reasonable vector proposals
  - One-at-a-time sampling simpler, but $x_{-i}$ constrain proposals for $x_i$
       - Most problematic when $x$ is smooth spatially
...
```
x[1:N] ~ dcar_normal(adj[1:L], weights[1:L], num[1:N], tau, c, zero_mean)
```

where `adj` gives info on which locations are neighbors of which locations, `weights` allows weighting of neighbors (often simply `1` as is case above), `num` is the number of neighbors for each location, and `tau` is the precision parameter.

This is an improper prior because the value of the prior doesn't change if you shift all the values by a constant: $x_1 + k, x_2 + k, \ldots, x_N + k$.
...
```{r, fig.cap = '', echo = FALSE, include=FALSE}
...
q <- classIntervals(respiratorydata_sp@data$SMR ,style = "fixed",fixedBreaks=seq(0.3, 1.7, by = 0.1))
pal <- temp.colors(12)
...
constants <- list(N = nregions, L = length(nbInfo$adj), 
                  ...
                  z = z, expected = respiratorydata_sp$expected)
data <- list(y = respiratorydata_sp$observed)
...
dists <- as.matrix(dist(respiratorydata_sp@data[ , c('easting', 'northing')]))
...
q <- classIntervals(xEstGP, style = "fixed",fixedBreaks=seq(-1.5, 1.5, by = 0.25))
pal <- temp.colors(12)
col <- findColours(q, pal)
...
# Vectorization and user-defined samplers
...
$$ x = B \beta $$
$$ \beta \sim MVN(0, \mbox{prec} = Q) $$


```{r}
# fit a simple GAM to get a sense for number of basis functions
if(FALSE) {
mod <- gam(y ~ log(expected) + z + s(locs, k = 40), family = 'poisson')
summary(mod)
fit <- predict(mod, type = 'terms')[,3]
}

locs <- as.matrix(respiratorydata_spatial@data[ , c('easting', 'northing')])/100000
y <- respiratorydata_spatial$observed
expected = respiratorydata_spatial$expected

K <- 40  # number of basis functions (might want)
out <- mgcv::jagam(y ~ log(expected) + s(locs, k = K) -1, family = 'poisson', na.action = na.pass, file = 'blank.jags')
B <- out$jags.data$X[ , -1]
K <- ncol(B)
## two components to ensure propriety of prior on coeffs (see Wood (2016) J Statistical Software)
...
constants <- list(Q1=Q1, Q2=Q2, B = B, K = K, N = nregions, z = z, zeros = rep(0, K), 
                            expected = respiratorydata_spatial$expected)
data <- list(y = respiratorydata_spatial$observed)
inits <- list(beta_z = 0)
## Use inits from jagam, but note that jagam call did not use 'expected' or 'z'
inits$beta_x <-  out$jags.ini$b[2:(K+1)]
inits$mu0 <- 0
inits$lambda <- out$jags.ini$lambda
...
conf <- configureMCMC(model)
conf$removeSamplers('beta_x[1:39]')
# Using non-default value of 0.25 for a tuning parameter of the adaptive block sampler helps a lot
# compared to default NIMBLE or to JAGS
conf$addSampler('beta_x[1:39]', 'RW_block', control = list(scale = 1, adaptFactorExponent = 0.25))
conf$addMonitors('x', 'beta_x')

MCMC <- buildMCMC(conf)
cMCMC <- compileNimble(MCMC, project = cModel)
samples <- runMCMC(cMCMC, niter = 100000, nburnin = 20000, thin = 80)

par(mfrow = c(1,4)
ts.plot(samples[ , 'lambda[1]'])
ts.plot(samples[ , 'lambda[1]'])
ts.plot(samples[ , 'x[20]'])
ts.plot(samples[ , 'x[40]'])

xCols <- grep('^x\\[', colnames(samples))
xEst <- colMeans(samples[ , xCols])

q <- classIntervals(xEst,style = "fixed",fixedBreaks=seq(-0.15, 0.15, by = 0.03))
pal <- temp.colors(10)
col <- findColours(q, pal)
plot(respiratorydata_sp, col = col, border = 'grey', lwd = 0.4)
```
...
For example, Stoner and Economou (2020) used mgcv-based spline terms in a NIMBLE model for precipitation, handling seasonality with cyclic splines.


# Appendix: using JAGS instead
