...
library(nimble)
DeerEcervi <- read.table('../examples/DeerEcervi/DeerEcervi.txt', header = TRUE)
...
# Overview
 ...
 - Features of NIMBLE and NIMBLE's model language useful for writing models
...
- Usage in the deer example. 

```{r}
DEcode1partial <- nimbleCode({
  for(i in 1:2) {
...
```{r, eval=FALSE}
farm.sd ~ dunif(0, 20)
farm.prec <- 1/farm.sd^2
for(i in 1:num.farms) {
  farm.effect[i] ~ dnorm(0, farm.prec)
}
```
...
Here `sex` and `farm.ids` are vectors of  membership indexes that are known in advance. Make sure to provide them in `constants` to avoid unnecessary computation (more later). 
...
The farm grouping above is 'ragged' (as is the sex assignment). If the number of observations per farm were constant ('regular'/'rectangular'), we could have something like this (ignoring sex and length):

```{r, eval=FALSE}
for(j in 1:num.farms) 
  for(i in 1:num.animals.per.farm) {
    logit(disease.probability[j, i]) <- farm.effect[j]
    Ecervi.01[j, i] ~ dbern(disease.probability[j, i])
  }
```
...
For typical MCMC samplers, `model$getDependencies('slope')` returns the nodes that need to be calculated when sampling `slope`.
...
Recall the likelihood component of the Deer model:

```{r, eval=FALSE}
for(i in 1:num.animals) {
  logit(disease.probability[i]) <- 
    sex.effect[ sex[i] ] +
    length.effect[ sex[i] ] * Length[i] +
    farm.effect[ farm.ids[i] ]
  Ecervi.01[i] ~ dbern(disease.probability[i])
```

Let's consider passing `farm.ids` either in `constants` or in `data`.
...
m_light <- nimbleModel(DEcode1, constants = DEconstants)
...
DEdata <- list(Ecervi.01 = DeerEcervi$Ecervi.01,
                    sex = DeerEcervi$Sex,
                    farm.ids = DeerEcervi$farm.ids)
m_heavy <-  nimbleModel(DEcode1, data = DEdata, constants = DEconstants)
```

Now let's look at the dependency (graph) structure.

```{r}
m_light$getDependencies('farm.effect[1]')
too_many_deps <- m_heavy$getDependencies('farm.effect[1]')
length(too_many_deps)
head(too_many_deps)
```

If `farm.ids` (and `sex`) are not known to NIMBLE to be constants, then all the observations could potentially depend on any element of `farm.effects`, `sex.effect`, `length.effect`.

Consider what would happen in a Metropolis-Hastings sampler for `farm.effect[1]` in the 'heavy' model.
...
1) Please run through the code from this module and use the commands discussed to understand the nodes and dependency structure of the Deer example.

2) Write code to fix the hyperparameters of the Deer example and given those fixed values, generate random farm effects and new observations, as would be needed for a simulation study:

```{r, eval=FALSE}
model$farm.sd <- 1
model$sex.effect <- c(1.5, 2.5)
model$length.effect <- c(0.3, 0.5)
