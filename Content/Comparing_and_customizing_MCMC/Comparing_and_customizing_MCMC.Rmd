...
date: "NIMBLE virtual workshop. June 2020"
output:
...
mcmcConf$addMonitors("farm.effect") # By default, top-level parameters are monitored. Let's get these random effects too.
DEmcmc1 <- buildMCMC(mcmcConf)
compiled <- compileNimble(DEmodel1, DEmcmc1) # Example of compiling model and MCMC in one call, which returns a list.
...
```{r, eval=doMCMC}
DEmodel1b <- DEmodel1$newModel(replicate = TRUE) # Fresh copy (in case code is used out of order in this doc)
mcmcConf <- configureMCMC(DEmodel1b)
params_for_slice <- "farm.effect[1:6]" # Notice: Not just one node
mcmcConf$printSamplers(params_for_slice)
mcmcConf$removeSamplers(params_for_slice) # Nodes will be expanded
expanded_params_for_slice <- DEmodel1b$expandNodeNames(params_for_slice)
...
compiled <- compileNimble(DEmodel1b, mcmc)
...
`compareMCMCs` is the name of a nearly complete R package available on github for managing performance comparisons among MCMC methods, particularly in `nimble`.  It can also run other MCMC "engines" via a plug-in system.  Currently there is a plug-in for JAGS and a draft plug-in for Stan.  Plug-ins for JAGS and OpenBUGS/WinBUGS (not yet implemented) can use the same model as `nimble` if it is compatible.
...
1. Same as above but without centering `Length` (just to make the blocking more interesting in this simple example)
...
DEconstants_uncentered$cLength <- DeerEcervi$Length # cLength is now mis-named, but it's easier than new model code.
```
...
mcmcResults_nimble_centered <- compareMCMCs(
...
```{r, include=FALSE}
mcmcResults_jags_centered <- list()
```

```{r, eval=(has_rjags&doComparisons)}
mcmcResults_jags_centered <- compareMCMCs(
...
mcmcResults_centered <- c(mcmcResults_nimble_centered, mcmcResults_jags_centered) ## These are lists of MCMCresult objects
make_MCMC_comparison_pages(mcmcResults_centered, modelName = "Deer_Ecervi_centered")
```

Results are [here](Deer_Ecervi_centered.html).
